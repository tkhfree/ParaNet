import type { Axios, AxiosPromise, AxiosRequestConfig, AxiosResponse, CancelToken } from 'axios'
...
import { isUndef, objectArrayFiledToString, recordValueTrim } from '@/utils/tools'
...
const noAuth = () => {
  ...
  window.location.hash = '#/project-manage'
}
...
instance.interceptors.request.use(
  config => {
    ...
    // 处理get请求参数值字符串左右空格
    if (config.params) {
      config.params = objectArrayFiledToString(recordValueTrim(config.params))
    }
