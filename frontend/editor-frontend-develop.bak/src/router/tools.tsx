...
import { usePageButtonAuth } from '@/stores'
...
export const getIsScreen = () => {
  //大屏path
  const pathList = ['data-screen']
  return pathList.every((i: any) => location.hash.indexOf(i) !== -1)
}

/**
 * 跳转有权限的页面hooks
2 */
export const useAuthPageNavigate = () => {
  const navigate = useNavigate()
  const hasPageAuth = usePageButtonAuth()
  const goAuthPage = (url: string) => {
    if (!hasPageAuth(url)) {
      modal.warning({ content: '无目标页面权限，无法跳转' })
      return
    }
    navigate(url)
  }
